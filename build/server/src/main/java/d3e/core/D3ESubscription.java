...
public class D3ESubscription implements FlowableOnSubscribe<DataStoreEvent> {
  ...
  public Flowable<D3ESubscriptionEvent<Invoice>> onInvoiceChangeEvent() {
    return this.flowable
        .filter((e) -> e.getEntity() instanceof Invoice)
        .map(
            (e) -> {
              D3ESubscriptionEvent<Invoice> event = new D3ESubscriptionEvent<>();
              event.model = ((Invoice) e.getEntity());
              event.changeType = e.getType();
              return event;
            });
  }

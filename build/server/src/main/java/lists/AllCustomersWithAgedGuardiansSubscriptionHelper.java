...
public class AllCustomersWithAgedGuardiansSubscriptionHelper
  ...
  private Field field;
  private Map<Long, List<Row>> a__guardian_id_Rows = MapExt.Map();

  @Async
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allCustomersWithAgedGuardiansImpl.getAsJson(field, event.nativeData);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allCustomersWithAgedGuardiansImpl.getNativeResult();
        ...
        ((Flowable<D3ESubscriptionEvent<Customer>>) subscription.onCustomerChangeEvent())
            .subscribe(
                (e) -> {
                  ...
                  List<Row> row0 = a__guardian_id_Rows.get(value.getId());
                  if (row0 == null) {
                    /*
  ...
  private List<NativeObj> createCustomerData(Customer customer) {
    ...
    row.set(0, customer.isUnderAge());
    row.set(1, customer.getGuardian().getAge());
    row.set(2, customer.getGuardian().getId());
    row.set(3, customer.getId());
    row.setId(3);
    data.add(row);
    return data;
  }
              ...
              .forEach(
                  (r) -> {
                    ...
                    if (wrappedBase != null) {
                      ...
                      updateData(ref0, a__guardian_id_Rows, r, false);
                    }
                  });
          break;
        }
      case Delete:
        {
          ...
          if (wrappedBase != null) {
            NativeObj ref0 = wrappedBase.getRef(2);
            updateData(ref0, a__guardian_id_Rows, delRow, true);
          }
          break;
        }
      case Insert:
        {
          ...
          if (wrappedBase != null) {
            NativeObj ref0 = wrappedBase.getRef(2);
            updateData(ref0, a__guardian_id_Rows, newRow, false);
          }
          break;
        }
      case Update:
  ...
  private void applyWhereGuardian(Row r, Customer customer) {
    NativeObj wrappedBase = r.row;
    NativeObj base = wrappedBase.getRef(3);
    boolean matched = base.getBoolean(0) && customer.getAge() >= 65l;
    if (!(matched)) {
      List<DataQueryDataChange> changes = ListExt.List();

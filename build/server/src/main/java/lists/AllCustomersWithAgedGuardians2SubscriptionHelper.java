...
public class AllCustomersWithAgedGuardians2SubscriptionHelper
  ...
  public void handleContextStart(DataQueryDataChange event) {
    updateData(event);
    try {
      event.data = allCustomersWithAgedGuardians2Impl.getAsJson(field, event.nativeData);
  ...
  private void loadInitialData() {
    ...
    change.nativeData = allCustomersWithAgedGuardians2Impl.getNativeResult();
  ...
  private List<NativeObj> createCustomerData(Customer customer) {
    ...
    row.set(0, customer.getGuardian().getId());
          ...
          this.output
              ...
              .forEach(
                  (r) -> {
                    ...
                    if (wrappedBase != null) {
                      NativeObj ref0 = wrappedBase.getRef(0);
  ...
  private void applyWhereGuardian(Row r, Customer customer) {
    ...
    boolean matched = base.getRef(0) != null && customer.getAge() >= 65l;
    if (!(matched)) {
      List<DataQueryDataChange> changes = ListExt.List();
